...
const NAV_ITEMS = [
  ...
  { label: 'Configuración', href: '/deportista/configuracion', icon: Settings },
];
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="p-5 border-b border-gray-800">
        <Link href="/deportista/dashboard" className="flex items-center gap-3">
        ...
        <div className="p-4 border-b border-gray-800">
          <div className="flex items-center gap-3">
            <Avatar className="h-10 w-10">
              <AvatarFallback className="bg-blue-600 text-white text-sm font-bold">{user.nombre?.[0]}{user.apellido?.[0]}</AvatarFallback>
            </Avatar>
          ...
          return (
            <Link key={item.href} href={item.href}
              className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-150 ${
                isActive ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'text-gray-400 hover:text-white hover:bg-gray-800/50'
              }`}>
...
export default function DeportistaLayout({ children }: { children: React.ReactNode }) {
          ...
          <div className="flex items-center gap-3">
            {user && (
              <>
                ...
                <Link href="/deportista/configuracion">
                  <Avatar className="h-9 w-9 cursor-pointer hover:ring-2 hover:ring-blue-500/50 transition-all">
                    <AvatarFallback className="bg-blue-600 text-white text-sm font-bold">{user.nombre?.[0]}{user.apellido?.[0]}</AvatarFallback>
                  </Avatar>
                </Link>
              </>
