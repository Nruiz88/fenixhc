...
export default function DeportistaNotificaciones() {
  const [notifs, setNotifs] = useState<any[]>([]);
  useEffect(() => {
    udb.select('notificaciones', '*').then(({ data }) => {
      setNotifs((data || []).filter((n: any) => n.destinatario_rol === 'deportista' || n.destinatario_rol === 'todos'));
    });
  }, []);
  ...
  return (
    <div className="space-y-6">
      <div><h1 className="text-2xl font-bold text-white">Notificaciones Deportivas</h1><p className="text-gray-400 text-sm mt-1">Avisos de entrenamientos y competencias</p></div>
      {notifs.length === 0 ? (
